...
char * find_the_word(char * sentence_ptr, char * searchWord_ptr, int * errorCode_ptr)
{
	//check everything for nulls and return null errors if necessary
	if (!sentence_ptr)
	{
		*errorCode_ptr = ERROR_NULL_SENTENCE_POINTER; // -1
		return NULL;
	}
	else if (!searchWord_ptr)
	{
		*errorCode_ptr = ERROR_NULL_SEARCH_POINTER; // -2
		return NULL;
	}
	...
	{
		//variables
		char * temp_searchWord_ptr = searchWord_ptr;
		char * temp_sentence_ptr = sentence_ptr;
		int successCount = 0;

		//search for the first letter of the search word in each letter in the sentence and then check the next strlen of the search word letters to see if they all match
		while (*sentence_ptr != '\0')
		{
			if (*sentence_ptr == *searchWord_ptr)
			{
				successCount = 0;
				temp_searchWord_ptr = searchWord_ptr;
				temp_sentence_ptr = sentence_ptr;

				//compare the letter at the current searchWord_ptr position to the letter at the current sentence_ptr position
				while (*temp_searchWord_ptr != '\0')
				{
					//if there is no match then break out of the loop
					if (*temp_sentence_ptr != *temp_searchWord_ptr)
					{
						break;
					}
					else
					{
						successCount++; //otherwise increment the successCount
					}

					//if successCount is equal to the length of the search word then return success and the current location of sentence_ptr
					if (successCount == strlen(searchWord_ptr))
					{
						*errorCode_ptr = ERROR_CODE_SUCCESS; // 0
						return sentence_ptr;
					}
					else
					{
						//otherwise increment the pointers and continue checking the letters
						temp_sentence_ptr++;
						temp_searchWord_ptr++;
					}
				}
	...
	//return this if nothing else was caught
	*errorCode_ptr = ERROR_SEARCH_NOT_FOUND; // -3
	return NULL;
}
